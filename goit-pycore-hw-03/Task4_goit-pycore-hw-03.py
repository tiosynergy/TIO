...
def get_upcoming_birthdays(user):  # / user - словник
...
# --------визначення дня народження колеги в поточному році----
    ...
    if user_birthday_this_year["birthday"].date() < now:
        ...
        # ------- Якщо наступний ДР у субботу, переноситься на понеділок ----------
        if user_birthday_next_year["birthday"].weekday() == 5:
            new_date_next_year = (
                user_birthday_next_year["birthday"].date() + timedelta(days=2)
            )
            congratulattion_date = {
                "name": user["name"],
                "birthday": datetime(
                    new_date_next_year.year,
                    new_date_next_year.month,
                    new_date_next_year.day
                ),
                "r_b": datetime.strptime(user["birthday"], "%Y.%m.%d").date()
            }
            print(f"Привітання {congratulattion_date['birthday'].date()}\n")

        # ------- Якщо наступний ДР у неділю, переноситься на понеділок ----------
        if user_birthday_next_year["birthday"].weekday() == 6:
            new_date_next_year = (
                user_birthday_next_year["birthday"].date() + timedelta(days=1)
            )
            congratulattion_date = {
                "name": user["name"],
                "birthday": datetime(
    ...
    if 0 <= (user_birthday_this_year["birthday"].date() - now).days <= 7:
        ...
        if user_birthday_this_year["birthday"].weekday() == 5:
            ...
            congratulattion_date = {
                "name": user["name"],
                "birthday": datetime(now.year, new_date.month, new_date.day),
                "r_b": datetime.strptime(user["birthday"], "%Y.%m.%d").date()
            }
...
if __name__ == "__main__":
    ...
    upcoming_birthdays = [
        get_upcoming_birthdays(user_birthday) for user_birthday in users
    ]
    ...
    for birthday in upcoming_birthdays:
        if birthday:
            print(
                f"{birthday['name']}, день народження {birthday['r_b']} : "
                f"привітати {birthday['birthday'].date()}\n"
            )